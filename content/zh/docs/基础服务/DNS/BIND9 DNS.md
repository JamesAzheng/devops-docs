...
## BIND9 配置文件说明
### 区域数据库
`/etc/bind/zones/db.internal.example.com`文件：
```bind
$TTL    86400
...
@       IN      NS      ns3.internal.example.com.   ; 如果有第三个
...
```
这个文件是 BIND9 的**正向解析区文件（Zone File）**，文件名通常是 `db.internal.example.com`（或类似），用于定义域名 `internal.example.com` 下所有主机名到 IP 地址的映射关系。
...
- **@**：代表当前区名本身，即 `internal.example.com.`（注意后面有点，域名全称以点结尾）。该值通过 zone 配置文件`/etc/bind/named.conf.local`中的 `zone "internal.example.com"` 和 `file` 指向的文件名结合来确定的。
- **IN**：表示 Internet 类（几乎总是 IN）。
- **SOA**：记录类型。
- **ns1.internal.example.com.**：这个域的主名字服务器（Primary NS），带点表示全称。
- **admin.internal.example.com.**：管理员邮箱（@ 替换成了点），即 admin@internal.example.com。
    - `ns1.internal.example.com.`、`admin.internal.example.com.` 这两个字段是 SOA 记录里必须填写的两个关键部分，不能省略。
- 括号内是 SOA 的参数：
...
- 写了三条，表示 `internal.example.com` 由 ns1、ns2、ns3 三台服务器共同负责。
...
- `ns1` 是简写，实际代表 `ns1.internal.example.com.`。
...
```
- 真正的业务主机记录。
- `s1.bj` 是简写，完整域名是 `s1.bj.internal.example.com.`。
- 表示访问 `s1.bj.internal.example.com` 时返回 IP 192.168.1.101。
- 分段写法（bj 层）是为了方便管理，实际解析时会自动补全成完整域名。

**总结这个文件的作用**
- 定义了域名 `internal.example.com` 的全部解析规则。
...
- 然后执行 `sudo rndc reload internal.example.com` 平滑重载。
- 可以继续添加更多记录，比如：
